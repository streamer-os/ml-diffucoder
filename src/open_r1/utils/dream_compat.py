...
- properly delegate parameters() and named_parameters() for optimizer compatibility
"""
...
class ModelCompatWrapper(nn.Module):
    ...
    - properly delegates parameters() and named_parameters() for optimizer compatibility
    """
    ...
    # -------------------------
    # Parameter delegation (crucial for optimizer compatibility)
    # -------------------------
    def parameters(self, recurse: bool = True) -> Iterator[nn.Parameter]:
        """
        Delegate to wrapped model's parameters. This is crucial for optimizers.
        """
        model = self.__dict__.get("model", None)
    ...
    def train(self, mode: bool = True):
        """Delegate train mode to wrapped model."""
        model = self.__dict__.get("model", None)
        if model is not None:
            try:
                model.train(mode)
            except Exception:
                logger.debug("Delegated model.train failed", exc_info=True)
        return super().train(mode)

    def eval(self):
        """Delegate eval mode to wrapped model."""
        model = self.__dict__.get("model", None)
        if model is not None:
            try:
                model.eval()
            except Exception:
                logger.debug("Delegated model.eval failed", exc_info=True)
        return super().eval()

    def zero_grad(self, set_to_none: bool = True):
        """Delegate zero_grad to wrapped model."""
        model = self.__dict__.get("model", None)
        if model is not None:
            try:
